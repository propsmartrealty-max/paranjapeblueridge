...
function testPseoUrls() {
  ...
  // Check Title Lengths (Google truncates after ~65-70 chars)
  const overLengthTitles = urls.filter(u => u.title.length > 70);
  if (overLengthTitles.length > 0) {
    console.warn(`⚠️ Warning: ${overLengthTitles.length} PSEO titles exceed 70 characters. Example: ${overLengthTitles[0].title}`);
  } else {
    console.log('✅ 100% of PSEO titles conform strictly to Google SERP snippet limits (<= 70 chars).');
  }
}
