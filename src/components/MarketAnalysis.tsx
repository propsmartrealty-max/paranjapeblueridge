    ...
    <section id="market" className="py-24">
      ...
      >
        <div className="text-center mb-16">
          ...
          <p className="text-xs sm:text-sm md:text-base text-text-muted max-w-2xl mx-auto mt-4 leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((s, i) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="p-6 sm:p-8 bg-gradient-to-b from-white/90 to-white/60 dark:from-slate-800/80 dark:to-slate-900/80 border border-gold/25 rounded-3xl shadow-lg hover:shadow-2xl hover:border-gold/60 transition-all group relative overflow-hidden"
            >
              ...
              <span className="text-[10px] text-gold font-bold uppercase tracking-widest block mb-3">{s.label}</span>
        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="p-6 sm:p-8 bg-gold/5 border border-gold/20 rounded-3xl shadow-inner flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Sparkles size={16} className="text-gold" />
                <span className="text-[10px] uppercase font-bold tracking-widest text-gold">Location Advantage</span>
              </div>
            ...
            <div className="space-y-3">
              {distances.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center p-3.5 bg-white/70 dark:bg-slate-900/60 rounded-2xl border border-gold/15 shadow-sm hover:border-gold/40 transition-all">
          ...
          <div className="p-6 sm:p-8 bg-gradient-to-b from-white/90 to-white/60 dark:from-slate-800/80 dark:to-slate-900/80 border border-gold/25 rounded-3xl shadow-lg flex flex-col justify-between">
            <div>
              <div className="flex flex-wrap justify-between items-center mb-6 gap-3">
                <div>
                  <span className="text-[10px] uppercase font-bold tracking-widest text-gold block">Financial Projection</span>
                  <h3 className="text-xl sm:text-2xl font-serif text-warm-white font-bold">ROI Yield Forecaster</h3>
                </div>
                <div className="flex gap-1.5 bg-gold/10 p-1 rounded-xl border border-gold/20">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="h-3 w-full bg-gold/15 rounded-full overflow-hidden p-0.5">
                    <motion.div 
                      initial={{ width: 0 }}
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="p-4 bg-gold/5 rounded-2xl border border-gold/15">
                    <span className="text-[10px] text-text-muted uppercase tracking-widest block mb-1">Rental Yield</span>
                    <span className="text-xl sm:text-2xl font-serif text-emerald-600 dark:text-emerald-400 font-bold">4.8% - 5.5%</span>
                    <span className="text-[9px] text-text-muted block mt-1">vs 2.8% Pune Average</span>
                  </div>
                  <div className="p-4 bg-gold/5 rounded-2xl border border-gold/15">
                    <span className="text-[10px] text-text-muted uppercase tracking-widest block mb-1">Metro Surge CAGR</span>
                    <span className="text-xl sm:text-2xl font-serif text-gold font-bold">12.4% / yr</span>
                    <span className="text-[9px] text-text-muted block mt-1">Post Line 3 Launch</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <a 
              href="#enquiry" 
              className="mt-6 w-full py-3.5 bg-gradient-to-r from-gold via-gold-light to-gold text-navy font-bold rounded-2xl text-center flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all shadow-lg text-xs uppercase tracking-wider"
            >
