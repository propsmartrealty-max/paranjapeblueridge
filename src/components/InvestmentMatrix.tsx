      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <div className="p-4 sm:p-6 bg-gradient-to-br from-gold/15 via-gold/10 to-gold/5 border border-gold/30 rounded-2xl sm:rounded-3xl shadow-md shrink-0">
            <span className="text-3xl sm:text-4xl font-serif text-gold font-bold block leading-none font-mono">15+</span>
            <span className="text-[9px] text-text-muted uppercase tracking-widest font-bold mt-1 block font-mono">IT Campuses within 3 KM</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {corporateMatrix.map((item, idx) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="p-5 bg-white/80 dark:bg-slate-900/70 border border-gold/25 rounded-2xl hover:border-gold/60 transition-all group shadow-sm hover:shadow-lg"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="w-10 h-10 bg-gold/10 rounded-xl flex items-center justify-center text-gold group-hover:bg-gold group-hover:text-navy transition-all border border-gold/25 shadow-sm">
                  <Building2 size={18} />
                </div>
                <span className="text-[8px] bg-gold/10 text-gold px-2 py-0.5 rounded-full uppercase font-bold tracking-wider font-mono">{item.hub}</span>
              </div>
              <h4 className="text-warm-white font-bold text-xs sm:text-sm mb-3 line-clamp-1 group-hover:text-gold transition-colors font-sans">{item.company}</h4>
        ...
        <div className="mt-10 p-6 sm:p-8 bg-gold/5 border border-gold/25 rounded-3xl flex flex-col lg:flex-row items-center justify-between gap-6 shadow-inner">
          ...
          <div className="flex gap-4 shrink-0">
            <div className="text-center px-5 py-3.5 bg-white/85 dark:bg-slate-900/85 rounded-2xl border border-gold/30 shadow-sm">
              <span className="text-xl sm:text-2xl font-serif text-gold font-bold block font-mono">1.5 Lac+</span>
              <span className="text-[8px] sm:text-[9px] text-text-muted uppercase tracking-widest font-bold">Tech Professionals</span>
            </div>
            <div className="text-center px-5 py-3.5 bg-white/85 dark:bg-slate-900/85 rounded-2xl border border-gold/30 shadow-sm">
              <span className="text-xl sm:text-2xl font-serif text-emerald-600 dark:text-emerald-400 font-bold block font-mono">₹ 85K+</span>
              <span className="text-[8px] sm:text-[9px] text-text-muted uppercase tracking-widest font-bold">Avg. IT Salary</span>
            </div>
          </div>
        </div>
