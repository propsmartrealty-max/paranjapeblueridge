...
export default function AtmosphereToggle() {
  ...
  return (
    <button 
      ...
      aria-label={`Switch to ${atmosphere === 'morning' ? 'Night' : 'Morning'} Theme`}
    >
      <motion.div 
      ...
      >
        {atmosphere === 'morning' ? <Sun className="w-[14px] h-[14px]" size={14} /> : <Moon className="w-[14px] h-[14px]" size={14} />}
      </motion.div>
      
      <div className="absolute inset-0 flex justify-between px-2 items-center opacity-30">
         <Sun className="w-3 h-3 text-gold" size={12} />
         <Moon className="w-3 h-3 text-gold" size={12} />
      </div>
    </button>
  );
}
