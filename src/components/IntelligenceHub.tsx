    ...
    <section id="market-hub" className="py-24">
      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <p className="text-xs sm:text-sm text-text-muted max-w-md leading-relaxed font-medium">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {insights.map((item, idx) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="p-6 sm:p-8 bg-gradient-to-b from-white/95 to-white/70 dark:from-slate-800/90 dark:to-slate-900/90 border border-gold/25 rounded-3xl relative overflow-hidden shadow-lg hover:shadow-2xl hover:border-gold/60 transition-all group flex flex-col justify-between"
            >
              ...
              <div>
                ...
                <h4 className="text-[10px] text-gold font-bold uppercase tracking-widest mb-3 font-mono">{item.title}</h4>
              </div>
              <p className="text-xs text-text-muted leading-relaxed font-medium">{item.description}</p>
            </motion.div>
          ))}
        </div>

        {/* DEEP DIVE FEATURE */}
        <div className="mt-12 p-8 sm:p-10 bg-gradient-to-r from-gold/10 via-gold/5 to-gold/10 border border-gold/30 rounded-3xl flex flex-col lg:flex-row items-center justify-between gap-8 shadow-inner">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <ShieldCheck size={16} className="text-emerald-500" />
              <span className="text-[10px] uppercase font-bold tracking-widest text-gold font-mono">Institutional Grade Research</span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 shrink-0">
            <div className="text-center p-5 bg-white/85 dark:bg-slate-900/85 rounded-2xl border border-gold/35 shadow-md">
              <div className="text-3xl sm:text-4xl font-serif text-gold font-bold font-mono">98.2%</div>
              <span className="text-[9px] text-text-muted uppercase tracking-widest font-bold block mt-0.5">Township Occupancy</span>
            </div>
            <a 
              href="#enquiry" 
              className="px-8 py-4 bg-gradient-to-r from-gold via-gold-light to-gold text-navy rounded-full font-bold uppercase text-xs tracking-widest hover:scale-105 transition-all shadow-xl hover:shadow-gold/30 shrink-0 text-center btn-sheen"
            >
