  ...
  return (
    <section id="connectivity" className="py-12">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
          ...
          <div>
            <a 
              ...
              className="inline-flex items-center gap-2.5 bg-gradient-to-r from-gold via-gold-light to-gold text-navy px-6 py-3.5 rounded-full text-xs font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-lg hover:shadow-gold/30 shrink-0"
        ...
        {/* MAP CONTAINER */}
        <div className="relative rounded-3xl overflow-hidden border border-gold/20 shadow-xl h-[450px] sm:h-[550px]">
          <iframe 
          ...
          <div className="absolute bottom-6 left-6 right-6 flex flex-wrap gap-2.5 pointer-events-none">
            {[
              ...
              <div key={i} className="px-4 py-2 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-gold/30 rounded-full text-warm-white font-bold text-[10px] uppercase tracking-wider flex items-center gap-2 shadow-lg">
                <MapIcon className="text-gold" size={12} />
                <span>{tag.label}: <strong className="text-gold font-mono">{tag.dist}</strong></span>
              </div>
            ))}
          </div>
        </div>
